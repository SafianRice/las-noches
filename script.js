...
const CLASS_COLOR = { A: '#d4af51', B: '#5b8fd0', C: '#c1523f', D: '#6b7186' };
const CLASS_LABEL = {
...
function attemptFrameUp(state, pawnId, targetId){
  ...
  const frameStrength = state.player.stats.influence * 0.4 + pawn.stats.charisme * 0.2 + (hasSecret ? 25 : 0) + rndInt(0, 10);
  const targetDefense = target.stats.reputation * 0.5 + target.stats.popularite * 0.3 + target.stats.sangFroid * 0.2;
...
function attemptSocialDestruction(state, targetId){
  ...
  const power = state.player.stats.influence * 0.4 + state.player.stats.popularite * 0.3 + state.player.stats.reputation * 0.2 + rndInt(0, 15);
  const resistance = target.stats.sangFroid * 0.5 + target.stats.popularite * 0.3 + target.stats.reputation * 0.2;
  const margin = power - resistance;
...
function applyExamOutcome(state, exam, strategy, result){
  ...
  const stakes = applyExamStakes(state, exam, result);

  return { tierData, pointsDelta, extraText, stakes };
}
...
function applyExamStakes(state, exam, result){
  ...
  // -- Risque d'expulsion : un(e) camarade en général, le joueur si sa situation est déjà critique --
  const expulsionChance = 0.06 + (severe ? 0.08 : 0) + (fails >= 2 ? 0.10 : 0) + (classInDanger ? 0.06 : 0);
  if (Math.random() < expulsionChance){
    const playerAtRisk = state.player.points < 150 && state.player.stats.reputation < 18 && fails >= 2;
    if (playerAtRisk && Math.random() < 0.35){
      state.gameOver = true;
...
const SCORE_RANKS = [
  ...
  { min: -Infinity,  label:'Dossier catastrophique',  color:'#d1483f' }
];
...
function computeFinalScore(state){
  ...
  if (s.player.flags.expelled) add('Expulsion de LNHS', -600);
...
const Game = {
  ...
  advanceWeek(){
    ...
    return { planningResult, examResult, weeklyEvent, specialWeek, crisisStage, marriageMoment, yearEnded, unlockedAchievements, goalsUpdate, consequenceResults, whiteRoomAlert };
  },
...
/** Panneau des objectifs narratifs à échéance (actifs, réussis, ratés). */
function goalsPanelHtml(state){
  ...
  const rows = goals.map(g => {
    ...
    if (!def) return '';
    if (g.status === 'success'){
      return `<div class="event-card"><div class="event-card__title">✅ ${escapeHtml(def.title)} <span class="event-card__tag">Accompli</span></div></div>`;
    }
    if (g.status === 'failed'){
      const rival = g.rivalNpcId ? getNpc(state, g.rivalNpcId) : null;
      return `<div class="event-card"><div class="event-card__title">❌ ${escapeHtml(def.title)} <span class="event-card__tag">Échoué</span></div>
        <div class="event-card__desc">${rival ? escapeHtml(rival.fullName + ' vous a devancé(e).') : 'L\'échéance est passée.'}</div></div>`;
    }
  ...
  return `<div class="panel"><div class="panel__title">Objectifs</div>${rows}</div>`;
}
...
function renderGameOverScreen(){
  ...
  const breakdownRows = items.length ? items.map(it => `
    <div class="rank-row">
      ...
      <div class="rank-row__points" style="color:${it.points>=0?'var(--azure)':'#d1483f'}">${it.points>0?'+':''}${it.points.toLocaleString('fr-FR')}</div>
    </div>`).join('') : `<div class="empty-state">Aucun point marquant.</div>`;

  el.innerHTML = `
    <h2 class="section-title">${expelled ? 'Dossier clos — Expulsion' : 'Fin de parcours — Diplôme LNHS'}</h2>
    <div class="panel">
...
function renderDashboard(){
  ...
  el.innerHTML = `
    ...
    <div class="kpi-row">
      ...
      <div class="kpi"><div class="kpi__label">Club</div><div class="kpi__value" style="font-size:1.1rem">${getClub(s.player.clubId) ? getClub(s.player.clubId).ic + ' ' + getClub(s.player.clubId).name : '—'}</div><div class="kpi__sub">${s.player.flags.studentCouncil ? 'Membre du conseil des élèves' : 'Onglet École pour rejoindre'}</div></div>
      <div class="kpi"><div class="kpi__label">Vie sentimentale</div><div class="kpi__value" style="font-size:1.1rem">${getActiveRomances(s).length}</div><div class="kpi__sub">${s.player.flags.relationshipStyle ? RELATIONSHIP_STYLE_LABELS[s.player.flags.relationshipStyle] : (getActiveRomances(s).length ? 'Statut indéfini' : 'Aucune romance active')}</div></div>
      <div class="kpi tt" data-tip="Rumeurs en circulation dans l'école, sur vous ou sur d'autres élèves."><div class="kpi__label">Rumeurs actives</div><div class="kpi__value">${(s.rumors||[]).length}</div><div class="kpi__sub">${(s.rumors||[]).some(r=>r.subjectId==='player') ? 'On parle de vous...' : 'Rien sur vous pour l\'instant'}</div></div>
    </div>

    <div class="grid-2">
      <div class="panel">
        <div class="panel__title">Cette semaine ${exam ? `<small>${isSurpriseExam ? 'Examen surprise !' : 'Un examen est programmé !'}</small>` : ''}</div>
        ${exam ? `
...
function openNpcModal(npcId){
  ...
  if (!pendingDate && rel.affinity >= 25 && npc.status === 'actif'){
  ...
  return `
    <div class="npc-card" data-npc="${n.id}">
      <div class="npc-card__badges">
        ...
        ${knowsSecret ? '<span class="badge-dot" title="Secret connu" style="background:#d1483f"></span>' : ''}
        ${s.rumors && s.rumors.some(r => r.subjectId === n.id) ? '<span class="badge-dot" title="Une rumeur circule" style="background:#8b6bb0"></span>' : ''}
      </div>
      <div class="npc-card__top">
...
function classSocialMapSvg(state, classId){
  ...
  const edgeSvg = npcEdges.map(e => {
    ...
    const color = e.v > 0 ? '#3f8f6f' : '#d1483f';
  ...
  return `
    ...
    <div class="social-map-legend">
      <span><i style="background:#3f8f6f"></i>Amitié entre élèves</span>
      <span><i style="background:#d1483f"></i>Rivalité entre élèves</span>
...
/** Point d'entrée : le joueur se rend réellement dans un lieu de l'école. */
...
function renderSchoolView(){
  ...
  el.innerHTML = `
    ...
    <p class="text-dim mt-2">Choisissez un lieu à visiter cette semaine.</p>
...
const JOURNAL_TYPE_LABELS = {
  ...
  rumeur:'Rumeurs', mystere:'Zone d\'ombre'
};

/** Icône/label selon la tendance d'une rumeur (mauvaise, bonne, ambiguë). */
function rumorValenceMeta(valence){
  if (valence === 'bad') return { ic:'🔥', color:'#d1483f', label:'Nuisible' };
  if (valence === 'good') return { ic:'✨', color:'#3f8f6f', label:'Flatteuse' };
  return { ic:'❔', color:'#5a6270', label:'Ambiguë' };
}
