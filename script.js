...
const SCORE_RANKS = [
  ...
  { min: -Infinity,  label:'Dossier catastrophique',  color:'#b23a3a' }
];
...
function renderGameOverScreen(){
  ...
  const breakdownRows = items.length ? items.map(it => `
    <div class="rank-row">
      ...
      <div class="rank-row__points" style="color:${it.points>=0?'var(--azure)':'#b23a3a'}">${it.points>0?'+':''}${it.points.toLocaleString('fr-FR')}</div>
...
function classSocialMapSvg(state, classId){
  ...
  const edgeSvg = npcEdges.map(e => {
    ...
    const color = e.v > 0 ? '#3f8f6f' : '#b23a3a';
  ...
  return `
    ...
    <div class="social-map-legend">
      ...
      <span><i style="background:#b23a3a"></i>Rivalité entre élèves</span>
...
function rumorValenceMeta(valence){
  if (valence === 'bad') return { ic:'🔥', color:'#b23a3a', label:'Nuisible' };
